...
metadata = read_tsv("tables/year2_sample_mapping_metaphlan.tsv")
metadata2 = metadata %>% 
...
relab = read_tsv("tables/year2_merged_abundance_table_species.txt")
...
wuni_fecal_black = as.dist(
  read.table("year2_merged_abundance_wunifrac_fecal_black.tsv"))
uuni_fecal_black = as.dist(
  read.table("year2_merged_abundance_uunifrac_fecal_black.tsv"))
wuni_oral_black = as.dist(
  read.table("year2_merged_abundance_wunifrac_oral_black.tsv"))
uuni_oral_black = as.dist(
  read.table("year2_merged_abundance_uunifrac_oral_black.tsv"))

wuni_fecal_white = as.dist(
  read.table("year2_merged_abundance_wunifrac_fecal_white.tsv"))
uuni_fecal_white = as.dist(
  read.table("year2_merged_abundance_uunifrac_fecal_white.tsv"))
wuni_oral_white = as.dist(
  read.table("year2_merged_abundance_wunifrac_oral_white.tsv"))
uuni_oral_white = as.dist(
  read.table("year2_merged_abundance_uunifrac_oral_white.tsv"))
...
brayf = vegdist(relab_fecal_filtered[,2:294], method = "bray")
jaccf = vegdist(relab_fecal_filtered[,2:294], method = "jaccard", 
                binary = T)
brayfb = vegdist(relab_fecal_filtered_b[,2:294], method = "bray")
brayfw = vegdist(relab_fecal_filtered_w[,2:294], method = "bray")
jaccfb = vegdist(relab_fecal_filtered_b[,2:294], method = "jaccard", 
                binary = T)
jaccfw = vegdist(relab_fecal_filtered_w[,2:294], method = "jaccard", 
                binary = T)
brayo = vegdist(relab_oral[,2:239], method = "bray")
jacco = vegdist(relab_oral[,2:239], method = "jaccard", 
                binary = T)
brayob = vegdist(relab_oral_b[,2:239], method = "bray")
brayow = vegdist(relab_oral_w[,2:239], method = "bray")
jaccob = vegdist(relab_oral_b[,2:239], method = "jaccard", 
                binary = T)
jaccow = vegdist(relab_oral_w[,2:239], method = "jaccard", 
                binary = T)
```
...
Fecal
```{r permanova_brayf}
adonis2(brayf ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, 
        data = relab_fecal_filtered, 
        permutations = 4999)

adonis2(brayf ~ subject_id*`Study period`, 
        data = relab_fecal_filtered, 
        permutations = 4999)

anova(betadisper(brayf, group = relab_fecal_filtered$Ethnicity))

anova(betadisper(brayf, group = relab_fecal_filtered$`Study period`))

anova(betadisper(brayf, group = relab_fecal_filtered$`Antibiotic use`))

anova(betadisper(brayf, group = relab_fecal_filtered$`Contraceptive use`))

anova(betadisper(brayf, group = relab_fecal_filtered$subject_id))
```

Oral
```{r permanova_brayo}
adonis2(brayo ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, data = relab_oral, 
        permutations = 4999)

adonis2(brayo ~ subject_id + `Study period`, 
        data = relab_oral, 
        permutations = 4999)

anova(betadisper(brayo, group = relab_oral$Ethnicity))

anova(betadisper(brayo, group = relab_oral$`Study period`))

anova(betadisper(brayo, group = relab_oral$`Antibiotic use`))

anova(betadisper(brayo, group = relab_oral$`Contraceptive use`))

anova(betadisper(brayo, group = relab_oral$subject_id))
```
...
brayfy2 <- ggplot(mds_bray_f_points2, aes(x = MDS1, y = MDS2, 
                                                 color = Ethnicity)) +
...
mds_bray_o_points2<-merge(x=mds_bray_o_points, y = relab_oral, 
                                  by.x = "row.names", by.y = "row.names")
brayoy2 <- ggplot(mds_bray_o_points2, aes(x = MDS1, y = MDS2, 
                                                 color = Ethnicity)) +
  geom_point(size=3) + scale_color_manual(
    values = c("White"="#87ceeb","Black"="#f5c4ca")) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), 
...
brayfsy2 <- ggplot(mds_bray_f_points2, aes(x = MDS1, y = MDS2, 
                                                 color = `Study period`)) +
  geom_point(size=3) + scale_color_manual(
...
brayosy2 <- ggplot(mds_bray_o_points2, aes(x = MDS1, y = MDS2, 
                                                 color = `Study period`)) +
  geom_point(size=3) + scale_color_manual(
...
tiff(file = "figures/nmds_plot_taxa_bray_all.tif", res = 300, width = 36,
     height = 14, units = "in")
legend1 = get_legend(brayfy1 + theme(legend.box.margin = 
                                        margin(0, 0, 0, 12)))
legend2 = get_legend(brayfsy1 + theme(legend.box.margin = 
                                        margin(0, 0, 0, 12)))
col1 = plot_grid(brayfy1 + brayoy1 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('A', 'E'), 
                 label_size = 20)
col2 = plot_grid(brayfsy1 + brayosy1 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('B', 'F'), 
                 label_size = 20)
col3 = plot_grid(brayfy2 + brayoy2 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('A', 'E'), 
                 label_size = 20)
col4 = plot_grid(brayfsy2 + brayosy2 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('A', 'E'), 
                 label_size = 20)
col5 = plot_grid(legend1 + legend2, nrow = 2, ncol =1, labels = c(""))
plot = plot_grid(col1, col2, col3, col4, col5, nrow = 1, ncol = 5, 
          rel_widths = c(1.5, 1.5, 1.5, 1.5, 0.75), 
          align = "h", axis = "t")
plot
dev.off()
```
...
Fecal
```{r permanova_jaccf}
adonis2(jaccf ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, data = relab_fecal_filtered, 
        permutations = 4999)

adonis2(jaccf ~ subject_id*`Study period`, 
        data = relab_fecal_filtered, 
        permutations = 4999)

anova(betadisper(jaccf, group = relab_fecal_filtered$Ethnicity))

anova(betadisper(jaccf, group = relab_fecal_filtered$`Study period`))

anova(betadisper(jaccf, group = relab_fecal_filtered$`Antibiotic use`))

anova(betadisper(jaccf, group = relab_fecal_filtered$`Contraceptive use`))

anova(betadisper(jaccf, group = relab_fecal_filtered$subject_id))
```

Oral
```{r permanova_jacco}
adonis2(jacco ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, data = relab_oral, 
        permutations = 4999)

adonis2(jacco ~ subject_id + `Study period`, 
        data = relab_oral, 
        permutations = 4999)

anova(betadisper(jacco, group = relab_oral$Ethnicity))

anova(betadisper(jacco, group = relab_oral$`Study period`))

anova(betadisper(jacco, group = relab_oral$`Antibiotic use`))

anova(betadisper(jacco, group = relab_oral$`Contraceptive use`))

anova(betadisper(jacco, group = relab_oral$subject_id))
```
...
```{r taxa_models_fecal}
fecaltax = wilcox.test(Acidaminococcus_intestini ~ Ethnicity, 
                        data = relab_fecal_filtered, 
                        paired = F, exact = F)
fecaltax
...
write.csv(taxa_Ethnicity, "tables/taxa_fecal_wilcox_Ethnicity.csv")
...
taxa_Ethnicity_corrected = bind_cols(taxa_Ethnicity_nona, 
                               as.data.frame(fdrtool(taxa_Ethnicity_nona[,3], 
                                                     statistic = "pvalue", 
                                                     plot = F)))
write.csv(taxa_Ethnicity_corrected, "tables/taxa_fecal_wilcox_Ethnicity_corrected.csv")
```
...
taxa_Ethnicity = bind_cols(as.data.frame(Ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(Ethnicity_pvaluematrix[,2]))
write.csv(taxa_Ethnicity, "tables/taxa_oral_wilcox_Ethnicity.csv")
taxa_Ethnicity_nona = filter(taxa_Ethnicity, taxa_Ethnicity[,3] != "NaN") 
taxa_Ethnicity_nona[,3] = as.numeric(as.character(taxa_Ethnicity_nona[,3]))
taxa_Ethnicity_corrected = bind_cols(taxa_Ethnicity_nona, 
                               as.data.frame(fdrtool(taxa_Ethnicity_nona[,3], 
                                                     statistic = "pvalue", 
                                                     plot = F)))
write.csv(taxa_Ethnicity_corrected, "tables/taxa_oral_wilcox_Ethnicity_corrected.csv")
```
