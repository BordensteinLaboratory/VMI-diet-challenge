...
for FILE in /data/bordenstein_lab/vmi/vmi_metagenomics/year2/shortread/shortbred/*shortbred_results.tsv
do
  SAMPLEID="${FILE%_shortbred.tsv}"
  cat $FILE | cut -f1,2 > ${SAMPLEID}_shortbred_subset.tsv
done

humann_join_tables --input /data/bordenstein_lab/vmi/vmi_metagenomics/year2/shortread/shortbred \
  --output /data/bordenstein_lab/vmi/vmi_metagenomics/year2/shortread/shortbred/all_shortbred_abundance.tsv \
