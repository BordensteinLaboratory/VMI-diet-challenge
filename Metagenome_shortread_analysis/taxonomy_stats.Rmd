...
Import metadata
```{r import_metay1, error=FALSE}
metadatay1 = read_tsv("year1_sample_mapping_metaphlan_noout.tsv")
metadata2y1 = metadatay1 %>% 
  mutate(Ethnicity = case_when(Ethnicity == "African_American" ~ "Black", 
                               Ethnicity == "Caucasian" ~ "White"))
```
```{r import_metay2, error=FALSE}
metadatay2 = read_tsv("year2_sample_mapping_metaphlan.tsv")
metadata2y2 = metadatay2 %>% 
  mutate(Ethnicity = case_when(Ethnicity == "African_American" ~ "Black", 
                               Ethnicity == "Caucasian" ~ "White"))
```

Import species-level relative abundance table with outliers (2f4, 4f2, 4f3, 4f4, 11f5) removed. Transpose and recalculate relative abundance to sum to 1 instead of 100.
```{r import_relaby1, error=FALSE}
relaby1 = read_tsv("year1_merged_abundance_table_species.txt")
relab1y1 = relaby1 %>% mutate_if(is.numeric, ~./100)
relab1y1_t = relab1y1 %>% gather(file, count, 2:193) %>% 
...
relaby2 = read_tsv("year2_merged_abundance_table_species.txt")
relab1y2 = relaby2 %>% mutate_if(is.numeric, ~./100)
relab1y2_t = relab1y2 %>% gather(file, count, 2:164) %>% 
  spread(body_site, count)
```
...
```{r filtery1}
relab_fecaly1 = relab1y1_t %>% left_join(metadata2y1, by = "file") %>% 
  filter(fecal_use == 1) 
relab_fecal_filteredy1 = relab_fecaly1 %>% 
  mutate(total = rowSums(relab_fecaly1[,2:555])) %>% 
  filter(total > 0) %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0)
relab_fecal_filtered_by1 = relab_fecal_filteredy1 %>% filter(Ethnicity == "Black")
relab_fecal_filtered_wy1 = relab_fecal_filteredy1 %>% filter(Ethnicity == "White")
relab_oraly1 = relab1y1_t %>% left_join(metadata2y1, by = "file") %>% 
  filter(oral_use == 1) %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0)
relab_oral_by1 = relab_oraly1 %>% filter(Ethnicity == "Black")
relab_oral_wy1 = relab_oraly1 %>% filter(Ethnicity == "White")
```


```{r filtery2}
relab_fecaly2 = relab1y2_t %>% left_join(metadata2y2, by = "file") %>% 
  filter(source == "fecal") 
relab_fecal_filteredy2 = relab_fecaly2 %>% 
  mutate(total = rowSums(relab_fecaly2[,2:496])) %>% 
  filter(total > 0) %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0)
relab_fecal_filtered_by2 = relab_fecal_filteredy2 %>% filter(Ethnicity == "Black")
relab_fecal_filtered_wy2 = relab_fecal_filteredy2 %>% filter(Ethnicity == "White")
relab_oraly2 = relab1y2_t %>% left_join(metadata2y2, by = "file") %>% 
  filter(source == "oral") %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0)
relab_oral_by2 = relab_oraly2 %>% filter(Ethnicity == "Black")
relab_oral_wy2 = relab_oraly2 %>% filter(Ethnicity == "White")
```
...
```{r beta_metricsy1, error=FALSE}
brayfy1 = vegdist(relab_fecal_filteredy1[,2:337], method = "bray")
jaccfy1 = vegdist(relab_fecal_filteredy1[,2:337], method = "jaccard")
brayfby1 = vegdist(relab_fecal_filtered_by1[,2:337], method = "bray")
brayfwy1 = vegdist(relab_fecal_filtered_wy1[,2:337], method = "bray")
jaccfby1 = vegdist(relab_fecal_filtered_by1[,2:337], method = "jaccard")
jaccfwy1 = vegdist(relab_fecal_filtered_wy1[,2:337], method = "jaccard")
brayoy1 = vegdist(relab_oraly1[,2:272], method = "bray")
jaccoy1 = vegdist(relab_oraly1[,2:272], method = "jaccard")
brayoby1 = vegdist(relab_oral_by1[,2:272], method = "bray")
brayowy1 = vegdist(relab_oral_wy1[,2:272], method = "bray")
jaccoby1 = vegdist(relab_oral_by1[,2:272], method = "jaccard")
jaccowy1 = vegdist(relab_oral_wy1[,2:272], method = "jaccard")
```

```{r beta_metricsy2, error=FALSE}
brayfy2 = vegdist(relab_fecal_filteredy2[,2:294], method = "bray")
jaccfy2 = vegdist(relab_fecal_filteredy2[,2:294], method = "jaccard")
brayfby2 = vegdist(relab_fecal_filtered_by2[,2:294], method = "bray")
brayfwy2 = vegdist(relab_fecal_filtered_wy2[,2:294], method = "bray")
jaccfby2 = vegdist(relab_fecal_filtered_by2[,2:294], method = "jaccard")
jaccfwy2 = vegdist(relab_fecal_filtered_wy2[,2:294], method = "jaccard")
brayoy2 = vegdist(relab_oraly2[,2:239], method = "bray")
jaccoy2 = vegdist(relab_oraly2[,2:239], method = "jaccard")
brayoby2 = vegdist(relab_oral_by2[,2:239], method = "bray")
brayowy2 = vegdist(relab_oral_wy2[,2:239], method = "bray")
jaccoby2 = vegdist(relab_oral_by2[,2:239], method = "jaccard")
jaccowy2 = vegdist(relab_oral_wy2[,2:239], method = "jaccard")
```
...
Fecal
```{r permanova_brayfy1}
adonis2(brayfy1 ~ Ethnicity, 
        data = relab_fecal_filteredy1, 
        permutations = 4999)
adonis2(brayfy1 ~ `Study period`, 
        data = relab_fecal_filteredy1, 
        permutations = 4999, by = "margin")
adonis(brayfby1 ~ `Study period`, 
        data = relab_fecal_filtered_by1, 
        permutations = 4999, by = "margin")
adonis(brayfwy1 ~ `Study period`, 
        data = relab_fecal_filtered_wy1, 
        permutations = 4999, by = "margin")
```
...
Oral
```{r permanova_brayoy1}
adonis2(brayoy1 ~ Ethnicity, data = relab_oraly1, 
        permutations = 4999)
adonis2(brayoy1 ~ `Study period`, 
        data = relab_oraly1, 
        permutations = 4999, by = "margin")
adonis2(brayoby1 ~ `Study period`, 
        data = relab_oral_by1, 
        permutations = 4999, by = "margin")
adonis2(brayowy1 ~ `Study period`, 
        data = relab_oral_wy1, 
        permutations = 4999, by = "margin")
```
...
```{r nmds_brayy1, echo=FALSE, error=FALSE}
mds_bray_fy1<-metaMDS(brayfy1, k=2, trymax=499)
mds_bray_f_pointsy1<-mds_bray_fy1$points
mds_bray_f_points2y1<-merge(x=mds_bray_f_pointsy1, 
                            y = relab_fecal_filteredy1, 
                                  by.x = "row.names", by.y = "row.names")

brayfy1 <- ggplot(mds_bray_f_points2y1, aes(x = MDS1, y = MDS2, 
                                                 color = Ethnicity)) +
  ...
  stat_ellipse(data = mds_bray_f_points2y1, aes(x = MDS1, y = MDS2, 
                                              fill = Ethnicity), 
                   alpha = 0.2, size = 0.8, geom = "polygon",
               linetype = 2,
               type = "t", level = 0.95) + 
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b><i>p</i> < 0.001, r<sup>2</sup> = 8.3% (All)</b><br><i>p</i> = 0.108, r<sup>2</sup> = 10.1% (Before)<br><b><i>p</i> < 0.001, r<sup>2</sup> = 8.5% (During)</b><br><b><i>p</i> = 0.009, r<sup>2</sup> = 9.3% (After)</b><br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.170",
...
mds_bray_oy1<-metaMDS(brayoy1, k=2, trymax=499)
mds_bray_o_pointsy1<-mds_bray_oy1$points
mds_bray_o_points2y1<-merge(x=mds_bray_o_pointsy1, y = relab_oraly1, 
                                  by.x = "row.names", by.y = "row.names")
brayoy1 <- ggplot(mds_bray_o_points2y1, aes(x = MDS1, y = MDS2, 
                                                 color = Ethnicity)) +
  geom_point(size=3) + scale_color_manual(
    values = c("White"="#87ceeb","Black"="#f5c4ca")) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), 
  ...
  theme(axis.title.x=element_text(size=rel(2)), 
        ...
        legend.title = element_text(size=rel(2)),
        legend.text = element_text(size = rel(1.8))) +
  stat_ellipse(data = mds_bray_o_points2y1, aes(x = MDS1, y = MDS2, 
                                              fill = Ethnicity), 
                   alpha = 0.2, size = 0.8, geom = "polygon",
               linetype = 2,
               type = "t", level = 0.95) + 
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b><i>p</i> = 0.007, r<sup>2</sup> = 8.4% (All)</b><br><i>p</i> = 0.135, r<sup>2</sup> = 8.7% (Before)<br><b><i>p</i> = 0.040, r<sup>2</sup> = 12.5% (After)</b><br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.158",
...
brayfsy1 <- ggplot(mds_bray_f_points2y1, aes(x = MDS1, y = MDS2, 
                                                 color = `Study period`)) +
  geom_point(size=3) + scale_color_manual(
  ...
  stat_ellipse(data = mds_bray_f_points2y1, aes(x = MDS1, y = MDS2, 
               ...
               type = "t", level = 0.95) + 
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.988, r<sup>2</sup> = 0.8% (Both)<br><i>p</i> = 0.983, r<sup>2</sup> = 1.7% (Black)<br><i>p</i> = 0.951, r<sup>2</sup> = 1.6% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.170",
...
brayosy1 <- ggplot(mds_bray_o_points2y1, aes(x = MDS1, y = MDS2, 
                                                 color = `Study period`)) +
  geom_point(size=3) + scale_color_manual(
  ...
  stat_ellipse(data = mds_bray_o_points2y1, aes(x = MDS1, y = MDS2, 
               ...
               type = "t", level = 0.95) + 
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.990, r<sup>2</sup> = 0.9% (Both)<br><i>p</i> = 0.879, r<sup>2</sup> = 3.7% (Black)<br><i>p</i> = 0.903, r<sup>2</sup> = 2.4% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.158",
...
```{r nmds_brayy2, echo=FALSE, error=FALSE}
mds_bray_fy2<-metaMDS(brayfy2, k=2, trymax=499)
mds_bray_f_pointsy2<-mds_bray_fy2$points
mds_bray_f_points2y2<-merge(x=mds_bray_f_pointsy2, 
                            y = relab_fecal_filteredy2, 
                                  by.x = "row.names", by.y = "row.names")

brayfy2 <- ggplot(mds_bray_f_points2y2, aes(x = MDS1, y = MDS2, 
                                                 color = Ethnicity)) +
  geom_point(size=3) + scale_color_manual(
    values = c("White"="#87ceeb","Black"="#f5c4ca")) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), 
  ...
  theme(axis.title.x=element_text(size=rel(2)), 
        ...
        legend.title = element_text(size=rel(2)),
        legend.text = element_text(size = rel(1.8))) +
  stat_ellipse(data = mds_bray_f_points2y2, aes(x = MDS1, y = MDS2, 
                                              fill = Ethnicity), 
                   alpha = 0.2, size = 0.8, geom = "polygon",
               linetype = 2,
               type = "t", level = 0.95) + 
...
mds_bray_oy2<-metaMDS(brayoy2, k=2, trymax=499)
mds_bray_o_pointsy2<-mds_bray_oy2$points
mds_bray_o_points2y2<-merge(x=mds_bray_o_pointsy2, y = relab_oraly2, 
                                  by.x = "row.names", by.y = "row.names")
brayoy2 <- ggplot(mds_bray_o_points2y2, aes(x = MDS1, y = MDS2, 
                                                 color = Ethnicity)) +
  geom_point(size=3) + scale_color_manual(
    values = c("White"="#87ceeb","Black"="#f5c4ca")) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), 
  ...
  theme(axis.title.x=element_text(size=rel(2)), 
        ...
        legend.title = element_text(size=rel(2)),
        legend.text = element_text(size = rel(1.8))) +
  stat_ellipse(data = mds_bray_o_points2y2, aes(x = MDS1, y = MDS2, 
                                              fill = Ethnicity), 
                   alpha = 0.2, size = 0.8, geom = "polygon",
               linetype = 2,
               type = "t", level = 0.95) + 
...
brayfsy2 <- ggplot(mds_bray_f_points2y2, aes(x = MDS1, y = MDS2, 
                                                 color = `Study period`)) +
  geom_point(size=3) + scale_color_manual(
    values = c("Before"="#238b45","During" = "#d94801", "After"="#2171b5")) + scale_fill_manual(
    values = c("Before"="#238b45","During" = "#d94801", "After"="#2171b5")) +
...
brayosy2 <- ggplot(mds_bray_o_points2y2, aes(x = MDS1, y = MDS2, 
                                                 color = `Study period`)) +
  geom_point(size=3) + scale_color_manual(
    values = c("Before"="#238b45","After"="#2171b5")) + 
  scale_fill_manual(values = c("Before"="#238b45","After"="#2171b5")) +
...
```{r nmds_save_bray}
tiff(file = "figures/nmds_plot_taxa_bray_e.tif", res = 150, width = 18, 
     height = 7, units = "in")
legend1 = get_legend(brayfy2 + theme(legend.box.margin = margin(0, 0, 0, 12)))
col1 = plot_grid(brayfy2 + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('A'), 
                 label_size = 20)
col2 = plot_grid(brayoy2 + theme(legend.position = "none"),
...
tiff(file = "figures/nmds_plot_taxa_bray_s.tif", res = 150, width = 18, 
     height = 7, units = "in")
legend1 = get_legend(brayfsy2 + theme(legend.box.margin = margin(0, 0, 0, 12)))
col1 = plot_grid(brayfsy2 + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('A'), 
                 label_size = 20)
col2 = plot_grid(brayosy2 + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('B'), 
                 label_size = 20) 
plot = plot_grid(col1, col2, legend1, nrow = 1, ncol = 3, 
          rel_widths = c(1.5, 1.5, 0.75), align = "h", axis = "t")
...
tiff(file = "figures/nmds_plot_taxa_bray_all.tif", res = 300, width = 30,
     height = 14, units = "in")
legend1 = get_legend(brayfy1 + theme(legend.box.margin = 
                                        margin(0, 0, 0, 12)))
legend2 = get_legend(brayfsy1 + theme(legend.box.margin = 
                                        margin(0, 0, 0, 12)))
col1 = plot_grid(brayfy1 + theme(legend.position = "none"), brayoy1 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('A', 'E'), 
                 label_size = 20)
col2 = plot_grid(brayfsy1 + theme(legend.position = "none"), brayosy1 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('B', 'F'), 
                 label_size = 20)
col3 = plot_grid(brayfy2 + theme(legend.position = "none"), brayoy2 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('C', 'G'), 
                 label_size = 20)
col4 = plot_grid(brayfsy2 + theme(legend.position = "none"), brayosy2 + theme(legend.position = "none"),
                 nrow = 2, ncol = 1, labels = c('D', 'H'), 
                 label_size = 20)
col5 = plot_grid(legend1, legend2, nrow = 2, ncol =1, labels = c(""))
plot = plot_grid(col1, col2, col3, col4, col5, nrow = 1, ncol = 5, 
          rel_widths = c(1.5, 1.5, 1.5, 1.5, 0.5), 
          align = "h", axis = "t")
plot
dev.off()
```
...
Fecal
```{r permanova_jaccf}
adonis2(jaccf ~ subject_id + `Study period` + subject_id*`Study period`, data = relab_fecal_filtered, 
        permutations = 4999)
...
Oral
```{r permanova_jacco}
adonis2(jacco ~ subject_id + `Study period`, data = relab_oral, 
        permutations = 4999)
...
jaccf <- ggplot(mds_jacc_f_points2, aes(x = MDS1, y = MDS2, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b><i>p</i> < 0.001, r<sup>2</sup> = 6.1% (All)</b><br><i>p</i> = 0.134, r<sup>2</sup> = 8.6% (Before)<br><b><i>p</i> < 0.001, r<sup>2</sup> = 6.3% (During)</b><br><b><i>p</i> = 0.018, r<sup>2</sup> = 7.3% (After)</b><br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.170",
...
jacco <- ggplot(mds_jacc_o_points2, aes(x = MDS1, y = MDS2, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b><i>p</i> = 0.005, r<sup>2</sup> = 12.2% (All)</b><br><i>p</i> = 0.134, r<sup>2</sup> = 8.1% (Before)<br><b><i>p</i> = 0.013, r<sup>2</sup> = 12.2% (After)</b><br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.167",
...
jaccfs <- ggplot(mds_jacc_f_points2, aes(x = MDS1, y = MDS2, 
  ...
  ggtitle("Jaccard - Fecal")  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.996, r<sup>2</sup> = 0.9% (Both)<br><i>p</i> = 0.998, r<sup>2</sup> = 1.5% (Black)<br><i>p</i> = 0.932, r<sup>2</sup> = 2.4% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.170",
...
jaccos <- ggplot(mds_jacc_o_points2, aes(x = MDS1, y = MDS2, 
  ...
  ggtitle("Jaccard - Oral")  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.996, r<sup>2</sup> = 1.3% (Both)<br><i>p</i> = 0.982, r<sup>2</sup> = 2.3% (Black)<br><i>p</i> = 0.671, r<sup>2</sup> = 6.1% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.167",
...
```{r nmds_save_jacc}
tiff(file = "figures/nmds_plot_taxa_jacc_e.tif", res = 150, width = 18, 
     height = 7, units = "in")
...
tiff(file = "figures/nmds_plot_taxa_jacc_s.tif", res = 150, width = 18, 
     height = 7, units = "in")
...
adonis2(wuni_oral ~ `Study period`, 
        data = relab_oral, 
        permutations = 4999, by = "margin")
pairwise.adonis(wuni_oral, 
                factors = relab_oral$`Study period`, 
                perm = 4999, 
                p.adjust.m='holm')
...
wunif <- ggplot(mds_wuni_f_points2, aes(x = MDS1, y = MDS2, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b><i>p</i> = 0.002, r<sup>2</sup> = 4.8% (All)</b><br><i>p</i> = 0.482, r<sup>2</sup> = 5.2% (Before)<br><b><i>p</i> = 0.012, r<sup>2</sup> = 6.0% (During)</b><br><i>p</i> = 0.065, r<sup>2</sup> = 7.7% (After)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.113",
...
wunio <- ggplot(mds_wuni_o_points2, aes(x = MDS1, y = MDS2, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.182, r<sup>2</sup> = 4.6% (All)<br><i>p</i> = 0.472, r<sup>2</sup> = 4.8% (Before)<br><i>p</i> = 0.124, r<sup>2</sup> = 11.4% (After)", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.103",
...
wunifs <- ggplot(mds_wuni_f_points2, aes(x = MDS1, y = MDS2, 
  ...
  ggtitle("Weighted UniFrac - Fecal")  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.865, r<sup>2</sup> = 1.0% (Both)<br><i>p</i> = 0.978, r<sup>2</sup> = 0.9% (Black)<br><i>p</i> = 0.261, r<sup>2</sup> = 5.7% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.113",
...
wunios <- ggplot(mds_wuni_o_points2, aes(x = MDS1, y = MDS2, 
  ...
  ggtitle("Weighted UniFrac - Oral")  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.876, r<sup>2</sup> = 1.0% (Both)<br><i>p</i> = 0.613, r<sup>2</sup> = 2.9% (Black)<br><i>p</i> = 0.471, r<sup>2</sup> = 6.1% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.103",
...
```{r nmds_save_wuni}
tiff(file = "figures/nmds_plot_taxa_wuni_e.tif", res = 150, width = 18, 
     height = 7, units = "in")
...
tiff(file = "figures/nmds_plot_taxa_wuni_s.tif", res = 150, width = 18, 
     height = 7, units = "in")
...
uunif <- ggplot(mds_uuni_f_points2, aes(x = MDS1, y = MDS2, 
  ...
  stat_ellipse(data = mds_uuni_f_points2, aes(x = MDS1, y = MDS2, 
               ...
               type = "t", level = 0.95)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b><i>p</i> < 0.001, r<sup>2</sup> = 7.6% (All)<br><i>p</i> = 0.016, r<sup>2</sup> = 11.2% (Before)<br><i>p</i> < 0.001, r<sup>2</sup> = 7.8% (During)<br><i>p</i> = 0.006, r<sup>2</sup> = 7.9% (After)</b>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.196",
...
uunio <- ggplot(mds_uuni_o_points2, aes(x = MDS1, y = MDS2, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.187, r<sup>2</sup> = 4.0% (All)<br><i>p</i> = 0.710, r<sup>2</sup> = 4.8% (Before)<br><i>p</i> = 0.346, r<sup>2</sup> = 6.8% (After)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.179",
...
uunifs <- ggplot(mds_uuni_f_points2, aes(x = MDS1, y = MDS2, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.994, r<sup>2</sup> = 0.9% (Both)<br><i>p</i> = 0.970, r<sup>2</sup> = 1.7% (Black)<br><i>p</i> = 0.992, r<sup>2</sup> = 1.4% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.196",
...
uunios <- ggplot(mds_uuni_o_points2, aes(x = MDS1, y = MDS2, 
  ...
  ggtitle("Unweighted UniFrac - Oral")  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<i>p</i> = 0.978, r<sup>2</sup> = 1.1% (Both)<br><i>p</i> = 0.965, r<sup>2</sup> = 1.8% (Black)<br><i>p</i> = 0.739, r<sup>2</sup> = 5.2% (White)<br>", 
           x = Inf, y = Inf, size = 4,
           hjust = 1.1, vjust = 1.1)  +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "Stress = 0.179",
...
```{r nmds_save_uuni}
tiff(file = "figures/nmds_plot_taxa_uuni_e.tif", res = 150, width = 18, 
     height = 7, units = "in")
...
tiff(file = "figures/nmds_plot_taxa_uuni_s.tif", res = 150, width = 18, 
     height = 7, units = "in")
...
brayfb = vegdist(relab_fecal_filtered_before[,2:294], method = "bray")
jaccfb = vegdist(relab_fecal_filtered_before[,2:294], method = "jaccard")
brayob = vegdist(relab_oral_before[,2:239], method = "bray")
jaccob = vegdist(relab_oral_before[,2:239], method = "jaccard")
```
...
brayfd = vegdist(relab_fecal_filtered_during[,2:294], method = "bray")
jaccfd = vegdist(relab_fecal_filtered_during[,2:294], method = "jaccard")
```
...
brayfa = vegdist(relab_fecal_filtered_after[,2:294], method = "bray")
jaccfa = vegdist(relab_fecal_filtered_after[,2:294], method = "jaccard")
brayoa = vegdist(relab_oral_after[,2:236], method = "bray")
jaccoa = vegdist(relab_oral_after[,2:236], method = "jaccard")
```
#### Bray-Curtis
Fecal
```{r permanova_brayfa}
...
```{r alpha_d}
shannond_fecal = diversity(relab_fecal_filtered[,2:294], index = "shannon")
shannond_oral = diversity(relab_oral[,2:236], index = "shannon")

shannone_fecal = shannond_fecal/log(specnumber(relab_fecal_filtered[,2:294]))
shannone_oral = shannond_oral/log(specnumber(relab_oral[,2:236]))

fecal_diversity = cbind(relab_fecal_filtered[,1], relab_fecal_filtered[,295:317], shannond_fecal, shannone_fecal)
fecal_diversity$`Study period` = as.factor(fecal_diversity$`Study period`)
oral_diversity = cbind(relab_oral[,1], relab_oral[,237:262], shannond_oral, shannone_oral)
...
tiff(file="figures/fecal_alpha_diversity_e.tif", res=150, width=9, height=4, units="in")
ggarrange(fecal_div_all, fecal_even_all, ncol = 2, nrow=1, 
          common.legend = TRUE, legend = "right", align = "h", 
          labels = c(""))
dev.off()

tiff(file="figures/oral_alpha_diversity_e.tif", res=150, width=9, height=4, units="in")
ggarrange(oral_div_all, oral_even_all, ncol = 2, nrow=1, 
          common.legend = TRUE, legend = "right", align = "h", 
          labels = c(""))
dev.off()

tiff(file="figures/fecal_alpha_diversity_s.tif", res=150, width=15, height=4, units="in")
ggarrange(fecal_div_study, fecal_even_study, ncol = 2, nrow=1, 
          common.legend = TRUE, legend = "right", align = "h", 
          labels = c(""))
dev.off()

tiff(file="figures/oral_alpha_diversity_s.tif", res=150, width=15, height=4, units="in")
...
```{r taxa_models_fecal}
Ethnicity_estimatematrix = mat.or.vec(295,2)
Ethnicity_pvaluematrix = mat.or.vec(295,2)
studyp_estimatematrix = mat.or.vec(295,2)
studyp_pvaluematrix = mat.or.vec(295,2)

relab_fecal_filtered = as.data.frame(relab_fecal_filtered)

for(i in 2:294) {
  variable = relab_fecal_filtered[,i]
  b = try(lm(variable ~ Ethnicity + `Study period`, 
                  data = relab_fecal_filtered))
  anova = anova(b)
...
```{r taxa_models_oral}
Ethnicity_estimatematrix = mat.or.vec(238,2)
Ethnicity_pvaluematrix = mat.or.vec(238,2)
studyp_estimatematrix = mat.or.vec(238,2)
studyp_pvaluematrix = mat.or.vec(238,2)
...
```{r taxa_graphs}
sig_taxa = read.csv("taxa_abundance_fecal.csv", header = T)
...
tiff(file="figures/sig_taxa_plots.tif", res=150, width=48, height=52, units="in") 
ggarrange(plotlist = plot, nrow = 13, ncol = 12,
          common.legend = T)
dev.off()
```