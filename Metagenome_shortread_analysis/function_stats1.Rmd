...
metadata_gene = read_tsv("tables/year1_sample_mapping_genes_noout.tsv") %>% 
...
gene_table = read_tsv("tables/year1_genefamilies_cog_cpm_unstratified.tsv")
...
write_csv(genes_fecal_filtered, "tables/gene_family_cog_cpm_fecal_year1.csv")
genes_fecal_filtered = read_csv("tables/gene_family_cog_cpm_fecal_year1.csv")
...
genes_oral_filtered = genes_oral %>% 
  ...
  filter(total > 0) %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0) 
write_csv(genes_oral, "tables/gene_family_cog_cpm_oral_year1.csv")
genes_oral_filtered = read_csv("tables/gene_family_cog_cpm_oral_year1.csv")
```
...
jaccf_gene = vegdist(genes_fecal_filtered[,2:42254], method = "jaccard", binary = T)
brayo_gene = vegdist(genes_oral_filtered[,2:33457], method = "bray")
jacco_gene = vegdist(genes_oral_filtered[,2:33457], method = "jaccard", binary = T)
...
metadata_args = read_tsv("tables/year1_sample_mapping_args_noout.tsv") %>% 
  mutate(Ethnicity = case_when(Ethnicity == "African_American" ~ "Black", 
                               Ethnicity == "Caucasian" ~ "White"))
```
...
args_fecal_filtered = read_csv("tables/arg_fecal_year1.csv")
...
args_oral_filtered = read_csv("tables/arg_oral_year1.csv")
```
