...
metadata = read_tsv("tables/year1_sample_mapping_metaphlan_noout.tsv")
metadata2 = metadata %>% 
...
relab = read_tsv("tables/year1_merged_abundance_table_species.txt")
...
Oral
```{r permanova_brayo}
adonis2(brayo ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, 
        data = relab_oral, 
        permutations = 4999)
...
Fecal
```{r permanova_jaccf}
adonis2(jaccf ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, 
        data = relab_fecal_filtered, 
        permutations = 4999)
...
Oral
```{r permanova_jacco}
adonis2(jacco ~ Ethnicity + `Study period` + `Antibiotic use` + `Contraceptive use` + subject_id, 
        data = relab_oral, 
        permutations = 4999)
